...
3. 当这个item到达\ ``FilesPipeline``\ 时，在\ ``file_urls``\ 字段中的URL列表会通过标准的Scrapy调度器和下载器来调度下载，并且优先级很高，在抓取其他页面前就被处理。而这个\ ``item``\ 会一直在这个pipeline中被锁定，直到所有的文件下载完成。
...
``ImagesPipeline``\ 使用\ `Pillow <https://github.com/python-pillow/Pillow>`__\ 来生成缩略图以及转换成标准的JPEG/RGB格式。因此你需要安装这个包，我们建议你使用Pillow而不是PIL。
...
如果你还需要更加复杂的功能，想自定义下载媒体逻辑，请参考\ `扩展媒体管道 <http://doc.scrapy.org/en/1.0/topics/media-pipeline.html#topics-media-pipeline-override>`__
